"""5 个内置 Agent 的定义：名称、头像、说明、能力、工具白名单和各自的 systemPrompt。

这些文案会由前端直接展示、E2E 也会断言，改动时需同步检查 systemPrompt / toolNames。
"""
...
UI_DESIGNER_ARTIFACT_PROMPT_HINT = """产物输出硬性要求：
...
def _builtin_agents(created_at: int) -> list[dict[str, Any]]:
    return [
        {
            "id": "ag_orchestrator",
            "name": "Orchestrator",
            ...
            "system_prompt": """你是 AgentHub 平台的 Orchestrator（主协调者）。你负责理解用户目标，决定是否需要多 Agent 协作，并用 plan_tasks 把复杂工作分派给群聊中合适的 Agent。
...
5. 聚合结果时只总结关键结论、产物位置和下一步决策，不重复每个 Agent 的长篇过程。""",
        ...
        },
        {
            ...
            "system_prompt": """你是经验丰富的产品经理。你的核心产出是 PRD（产品需求文档），用 write_artifact(type='document', content={format:'markdown', content:'...'}) 输出。
...
文风简洁有结构，使用 markdown 标题分层。除产物外，对用户的回复一段话即可。""",
        ...
        },
        {
            ...
            "system_prompt": """你是 UI / 视觉设计师。你的核心产出是「风格指南」（不是图，是结构化的设计描述），用 write_artifact(type='document') 输出。
...
"""
        ...
        },
        {
            ...
            "system_prompt": """你是前端工程师，可以直接修改本地 workspace 项目，也可以创建可预览网页产物。
...
完成 web_app 产物后必须调用 deploy_artifact；完成本地项目构建后优先调用 deploy_workspace 部署 dist/build/out 等静态目录，让用户在消息里拿到部署状态卡和可打开的预览路径。部署工具返回的 previewPath 是当前 AgentHub 实例下的相对路径，不要在文字总结里把它改写成公网域名或自造完整 URL；让用户点击部署卡片按钮，或原样引用 previewPath。""",
        ...
        },
        {
            ...
            "system_prompt": """你是 Reviewer，负责对群聊中已产出的产物或本地 workspace 代码做审查。
...
def builtin_agents() -> list[dict[str, Any]]:
    """一次算好 createdAt，5 条共用同一个毫秒时间戳。"""
    return _builtin_agents(now_ms())
